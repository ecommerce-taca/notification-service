...
import { NotificationService, ReadStatusFilter } from '../notification/notification.service';
...
export class InAppService {
  ...
  async list(
    ...
    const { items, total } = await this.notificationService.findPaged({
  ...
  async markRead(userId: string, notificationId: string): Promise<void> {
    const notification = await this.notificationService.findByIdAndRecipient(notificationId, userId);
  ...
  async markAllRead(userId: string, before?: string): Promise<number> {
    ...
    return this.notificationService.markAllRead(userId, beforeDate);
  }
